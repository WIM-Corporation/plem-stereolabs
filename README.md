...
- `${name}_camera_link` -- 카메라 하우징 중심 (마운트 기준점)
- `${name}_left_camera_frame_optical` -- 좌안 광학 중심 (Hand-Eye 캘리브레이션 기준)
- `${name}_left_camera_frame_optical` -- 뎁스 광학 프레임 (ROS 규약: z-전방, x-우측, y-하방)
...
1. `urdf/{model}.xacro` 작성 -- 매크로 이름은 `zed_camera` 규칙을 따른다.

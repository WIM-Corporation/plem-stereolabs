...
zed-ros2-wrapper 사용 시 필수 규칙. 전체 파라미터/서비스는 `/zed-docs` 스킬의 `ros2-api-reference.md` 참조.
...
> ZED 드라이버는 RELIABLE로 발행한다. 구독자도 `RELIABLE`로 매칭 권장. 프레임 드롭 허용 시에만 `BEST_EFFORT` 가능.
...
TF 통합 상세: `/zed-docs` 스킬의 `robot-integration.md` 참조.
...
CPU 과부하: `grab_compute_capping_fps`, `point_cloud_freq` 동적 조절. 해상도: `pub_downscale_factor: 2.0`. 상세: `/zed-docs` 스킬의 `optimization.md`
...
상세 문서는 `/zed-docs` 스킬에서 제공한다 (SSOT).

| `/zed-docs` 스킬 문서 | 내용 |
|----------------------|------|
| `ros2-api-reference.md` | 전체 토픽/서비스/파라미터 표 |
| `usage-guide.md` | RViz, SVO, OD 데모, param_overrides 상세 |
| `yolo-config.md` | YOLO ONNX config, class 정의 |
| `dds-network.md` | DDS/커널/MTU 네트워크 튜닝 |
| `optimization.md` | Frequency/latency/ROI 성능 최적화 |
| `robot-integration.md` | Manipulator TF, multi-camera, streaming |
| `recording.md` | SVO/rosbag 녹화·재생·벤치마크 |
| `nv12-access.md` | NV12 zero-copy, GMSL2 raw access |
